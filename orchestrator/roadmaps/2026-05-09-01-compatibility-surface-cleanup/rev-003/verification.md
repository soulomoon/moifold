...
- Grep/read back the roadmap id
  `2026-05-09-01-compatibility-surface-cleanup`, roadmap revision `rev-003`,
  roadmap style `strategy-backlog`, and activation metadata for
  `roadmap_revision`: `rev-003` and the `rev-003` roadmap directory.
- Read back that milestones 001-007 are complete with progress pointers
  through rounds 052-071.
- Read back that milestone 008 is held after round 072, not complete, and that
  the hold does not approve any removal or compatibility behavior change.
...
- Read back that milestone 009 is dependency-ready only on the explicit
  round-072 hold path or on later exact approved removal rounds, and that
  `direction-023-final-compatibility-surface-report` is the next lawful hold
  path dispatch after rev-003 activation.
- Confirm `rev-001` and `rev-002` remain intact unless the update
  intentionally records moved history in `roadmap-history.md`.
...
- Follow-up evidence before removal: rev-003 carries forward the rev-002
  requirement for import-facade evidence, runtime compatibility evidence, and
  external operator/downstream inventory before final selected removal work.
- Hold before final report: if no exact removal surface is lawful, milestone
  008 must remain held and milestone 009 may proceed only to final
  hold/report work that preserves every blocker and states that no removal was
  approved.
...
- Reviewers must verify that rev-003 does not upgrade any round-058 candidate,
  round-071 unavailable evidence, or round-072 held removal direction to
  current migration, deprecation, removal, package publication, upload, or
  release approval.
...
- A held `milestone-008-gated-compatibility-removals` is a lawful predecessor
  for milestone 009 final hold/report work, but it is not removal completion.
