...
- Deferred alternatives: removal extraction is not lawful because round 072
  ...
  and deferred surfaces plus blockers.

## Outcome Boundaries

- In scope: carried-forward import-facade follow-up evidence, runtime
  compatibility-file follow-up evidence, external operator/downstream
  inventory, the approved milestone-008 hold, final reporting, and explicit
  done-or-hold closeout.
...
- Rev-002 added follow-up evidence before removal. Round 058 candidates remain
  evidence gaps, not removal approvals.
- Rev-003 records round 072's approved no-lawful-removal hold as a coordination
  state. A held milestone 008 does not approve removal and does not count as
  removal completion, but it is a lawful predecessor for milestone 009 final
  report/terminal hold work.
...
- Gated removals are allowed only after milestones 005-007 complete and only
  ...
  reviewer approves the exact removal evidence. If no such surface exists, the
  controller must select the explicit hold/final-report path rather than infer
  removal approval from dependency reachability.
...
### 8. [held] Perform Gated Compatibility Removals
...
unapproved compatibility behavior drift. Hold signal: an approved status round
records that no exact surface currently satisfies every removal gate and exact
reviewer approval.
...
removal gate and exact reviewer approval. Round 072's roadmap update was then
approved and merged in `0821ca8907a49070a4ec4b064427633ef4a6a59e`, leaving
rev-002 with milestone 008 held and milestone 009 still dependency-blocked.

Rev-003 preserves milestone 008 as held rather than removal-complete. This
hold does not approve deprecation, migration, removal, package publication,
...
remains unavailable or blocked evidence, not removal approval. The hold is
only a coordination predecessor for milestone 009 final reporting and terminal
hold work.
...
completed by later exact approved removal rounds. The rev-003 hold path is
currently satisfied by round 072 and this roadmap revision; removals are not
required for final hold/report selection.
...
- Direction id: `direction-023-final-compatibility-surface-report`
  ...
  rounds are complete. The current path is the approved hold path.
  Boundary notes: do not imply package publication, release approval,
...
- Direction id: `direction-024-terminal-cleanup-gate`
  ...
  terminal completion on the current path means a reviewed hold, not removal
  completion or publication approval.
