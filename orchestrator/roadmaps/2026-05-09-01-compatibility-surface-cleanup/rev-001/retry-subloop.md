...
`orchestrator/roadmaps/2026-05-09-01-compatibility-surface-cleanup/rev-001/`.
...
- Same-round retry is allowed when inventory evidence misses a compatibility
  surface, caller, writer, reader, fixture, repair path, or healthcheck path;
  the retry must expand the evidence rather than narrow the scope silently.
- Same-round retry is allowed when a policy round overstates deprecation or
  removal readiness; the retry must narrow the claim to evidence-backed gates.
...
- Same-round retry is allowed when a final removal round discovers an
  unsupported remaining user or missing old-log evidence; the round must return
  to implementation or plan and may reclassify the surface as deferred.
...
- `REJECTED` because the plan is incomplete, skips required evidence, or
  selects removal before the final removal milestone returns the same round to
  `plan`.
...
- `REJECTED` because inventory or policy overclaims readiness returns the same
  round to `implement` unless the reviewer says the plan must be narrowed.
...
- Near the end of the initial todo list, a roadmap update must either expand
