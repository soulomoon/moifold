...
`orchestrator/roadmaps/2026-05-07-00-workflow-kernel-indexing/rev-003/`.
...
- Same-round retry is allowed for issue-planning indexed ports when graph validation, graph normalization, daemon dry-run/execute, systemError retry/block, ready-issues fanout, or compatibility writes diverge from the existing moifold path; the retry must restore parity rather than moving concrete policy into indexed core.
...
- `REJECTED` because an indexed path changes event schema, golden replay, dry-run text, daemon result shape, action ordering, request-id progression, compatibility writes, graph normalization, scope validation, systemError retry/block behavior, or fanout boundaries returns the same round to `implement`.
