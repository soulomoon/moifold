...
`orchestrator/roadmaps/2026-05-07-00-workflow-kernel-indexing/rev-004/`.
...
- Same-round retry is allowed when an indexed port breaks compatibility parity, but the fix must return to the selected compatibility contract rather than broadening the indexed API.
- Same-round retry is allowed for IssueImplement indexed ports when plan-mode behavior, PR discovery, PR creation or reuse, PR body update, implementation turn handling, review handoff, PR merge waiting, post-merge review, follow-up, issue close, child lifecycle reporting, daemon dry-run/execute, detailed transaction failures, compatibility writes, action ordering, request-id progression, or invalid-observation behavior diverges from the existing moifold path; the retry must restore parity rather than moving concrete policy into indexed core.
...
- `REJECTED` because an indexed path changes event schema, golden replay, dry-run text, daemon result shape, action ordering, request-id progression, compatibility writes, issue-plan recording, PR URL rendering, PR discovery/link validation, branch advancement, post-merge follow-up behavior, issue close behavior, child lifecycle ownership, or repair behavior returns the same round to `implement`.
